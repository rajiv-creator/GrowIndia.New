// components.js v17 — shared app/db/ui helpers (no build step)
(function(){
  if(!window.supabase){
    console.error('Supabase client not found. Ensure /config.js sets window.supabase before this file.');
  }
  ...
  app.requireAuth = async function(opts={}){
    ...
    if(!data.session){
      window.location.href = (opts.redirectTo || '/login.html');
  ...
  db.updateSiteCopy = async function(key, content){
    return await sb.from('site_copy').upsert({ key, content, updated_at: new Date().toISOString() }, { onConflict:'key' }).select().maybeSingle();
  };
